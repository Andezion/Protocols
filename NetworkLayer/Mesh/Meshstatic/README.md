...
Дальше нам нужно как-то сжать и зашифровать данные, для этого используется протобаф и AES-128. Протобаф позволяет сжать данные до минимального размера, а AES-128 позволяет зашифровать данные с использованием 128-битного ключа. В итоге мы получаем зашифрованный массив байт, который можно отправить по радиоканалу.

Чтобы отправить сообщение, нужно создать объект Data, заполнить его полями и вызвать функцию sendData(). Функция sendData() создаёт объект MeshPacket, заполняет его полями и вызывает функцию sendMeshPacket(). Функция sendMeshPacket() шифрует данные и отправляет их по радиоканалу. На стороне получателя, функция receiveMeshPacket() принимает пакет, расшифровывает данные и вызывает функцию handleData(), которая обрабатывает полученные данные. 
