...
// Флаг который говорит, что при создании сокета нужно установить опцию SO_REUSEADDR (позволяет нескольким сокетам привязываться к одному порту, полезно для многопроцессных серверов)
