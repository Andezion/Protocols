...
// как узел Yggdrasil, ожидающий входящий пиринг).
int main(void) {
    ...
    if (ygg_send_metadata(conn, &node) != 0) { fprintf(stderr, "send metadata failed\n"); return 1; }

    unsigned char peer_ed_pk[crypto_sign_PUBLICKEYBYTES];
    if (ygg_recv_metadata(conn, peer_ed_pk) != 0) { fprintf(stderr, "recv metadata failed\n"); return 1; }
